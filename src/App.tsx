...
function ClientRoadmap({ orders, user, initialSearch = '', role, isPublic = false  }: 
  ...
  const isReadOnlyItems = foundOrder && [
    'En cuadro', 'En montaje', 'En impresión', 'En sublimación', 'En corte',
    'En confección', 'En empaque', 'En despacho', 'Entregado'
  ...
  const getDisplayStatus = (status: OrderStatus) => {
    if (['En diseño', 'Versión enviada', 'Corrección solicitada'].includes(status)) return 'En espera por aprobación';
    if (status === 'Arte final cargado') return 'Diseño aprobado';
    if (isPublic && ['En sublimación', 'En corte', 'En confección', 'En empaque'].includes(status)) return 'En producción';
    return status;
  };
 
  const getNormalizedStatus = (status: OrderStatus) => {
    if (isPublic) {
      if (['En diseño','Versión enviada','Corrección solicitada','Diseño aprobado','En cuadro','En montaje','En impresión','En sublimación','En corte','En confección','En empaque'].includes(status)) return 'En producción' as any;
    } else {
      if (['Diseño aprobado','En cuadro','En montaje','En impresión'].includes(status)) return 'En diseño';
  ...
  const getStepTime = (step: OrderStatus, index: number) => {
    ...
    if (index === 0) {
      ...
      const stepAliases: Partial<Record<OrderStatus, string[]>> = { 'En sublimación': ['En sublimación','En impresión'] };
      const aliases = stepAliases[step] || [step];
      const historyEntry = [...foundOrder.history].reverse().find(h => h.action === 'Cambio de Estado' && aliases.some(a => h.details.includes(a)));
    ...
    if (index < currentStepIndex) {
      ...
      const nextHistoryEntry = [...foundOrder.history].reverse().find(h => h.action === 'Cambio de Estado' && h.details.includes(nextStep));
      if (nextHistoryEntry) {
        endTime = new Date(nextHistoryEntry.created_at);
      } else {
        for (let i = index + 1; i <= currentStepIndex; i++) {
          const futureEntry = [...foundOrder.history].reverse().find(h => h.action === 'Cambio de Estado' && h.details.includes(steps[i]));
          if (futureEntry) { endTime = new Date(futureEntry.created_at); break; }
        }
      }
  ...
  const getDeliveryMessage = (order: any) => {
    ...
    if (status === 'Diseño aprobado' && !hasItems) return 'Completa la lista de uniformes para programar entrega';
    switch (status) {
      case 'Abono pendiente':   return 'Esperando confirmación de pago';
      case 'Abono confirmado':  return 'Carga la información requerida. <br>Estas imagenes son de importancia ya que son la referencia de la orden al diseñador.';
      case 'En diseño':         return 'Estamos en construcción de tu prediseño.<br> Una vez aprobada esta de tú parte podremos seguir con el siguiente paso.';
      case 'Versión enviada':   return 'Pendiente de aprobación del cliente';
      case 'Corrección solicitada': return 'Aplicando correcciones al diseño';
      default: return 'Fecha por definir';
    }
  };
 
  const productionSubSteps = ['En diseño','Versión enviada','Corrección solicitada','Diseño aprobado','En cuadro','En montaje','En impresión','En sublimación','En corte','En confección','En empaque'];
  const productionProgressIndex = foundOrder ? productionSubSteps.indexOf(foundOrder.status) : -1;
  const productionPercent = productionProgressIndex >= 0 ? Math.round(((productionProgressIndex + 1) / productionSubSteps.length) * 100) : 0;
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mx-auto space-y-16">
      ...
      {foundOrder ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-surface p-12 border border-border-custom shadow-2xl relative overflow-hidden">
            ...
            <div className="flex flex-col items-center md:items-end gap-4">
              <div className="text-center md:text-right">
                ...
                <h4 className="text-3xl font-black tracking-tighter transition-all duration-500"
                  ...
                  {isPostDesign ? (foundOrder.delivery_date ? format(new Date(foundOrder.delivery_date), 'dd MMM, yyyy') : 'PENDIENTE') : 'Por confirmar'}
                </h4>
                {!isPostDesign && (
                  <div className="mt-4 flex items-start gap-3 bg-red-500/10 border border-red-500/20 text-red-500 px-5 py-4 rounded-2xl shadow-lg animate-fade-in">
                    ...
                    <div className="text-left">
                      <p className="text-[10px] font-black uppercase tracking-widest">Información importante</p>
                      <p className="text-xs font-semibold leading-relaxed" dangerouslySetInnerHTML={{ __html: getDeliveryMessage(foundOrder) }} />
                    </div>
                  </div>
                )}
              </div>
          ...
          {showReceiptModal && lastPayment && (
            <ReceiptModal order={foundOrder} payment={lastPayment} onClose={() => setShowReceiptModal(false)} />
          )}
            ...
            <div className="relative flex justify-between">
              {steps.map((step, i) => {
                ...
                const isInDesignSubPhase   = currentOrderStatus && (
                  ...
                  ['Diseño aprobado','En diseño','Versión enviada','Corrección solicitada','En cuadro','En montaje','En impresión'].includes(currentOrderStatus)
                );
                return (
                  <div key={step} className="flex flex-col items-center gap-6 relative">
                      ...
                      <div className="absolute top-14 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 mt-2 w-32">
                        ...
                        <div className="bg-surface border border-border-custom rounded-2xl p-2 space-y-1.5 w-full shadow-lg">
                          {designSubSteps.map((sub) => {
                            const subStatusOrder: OrderStatus[] = ['Diseño aprobado','En cuadro','En montaje','En impresión'];
                      ...
                      <div className="absolute top-14 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 mt-2 w-32">
                        ...
                        <div className="bg-surface border border-border-custom rounded-2xl p-2 space-y-1.5 w-full shadow-lg">
                          {(['En sublimación','En corte'] as OrderStatus[]).map((sub) => {
                            const subCompleted = currentStepIndex > i || (sub === 'En sublimación' && ['En corte','En confección','En empaque','En despacho','Entregado'].includes(foundOrder?.status || ''));
                            const subCurrent   = foundOrder?.status === sub;
                            return (
                              <div key={sub} className="flex items-center gap-1.5">
                                <div className={cn("w-4 h-4 rounded-full flex items-center justify-center shrink-0 transition-all", subCompleted || currentStepIndex > i ? "bg-accent text-white" : subCurrent ? "bg-accent/30 border-2 border-accent" : "bg-surface-hover border border-border-custom")}>
                                  {(subCompleted || currentStepIndex > i) && <CheckCircle2 size={10} />}
                                  {subCurrent && <div className="w-1.5 h-1.5 rounded-full bg-accent"></div>}
                                </div>
                                <span className={cn("text-[8px] font-black uppercase tracking-wider", subCompleted || currentStepIndex > i ? "text-accent" : subCurrent ? "text-foreground-main" : "text-foreground-muted/30")}>{sub === 'En sublimación' ? 'En Sublimación' : 'En Corte'}</span>
                              </div>
                            );
                          })}
                        </div>
                      ...
                      <div className="absolute top-14 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 mt-2 w-36">
                        ...
                        <div className="bg-surface border border-border-custom rounded-2xl p-3 w-full shadow-lg space-y-2">
                          ...
                          <div className="h-1.5 w-full bg-surface-hover rounded-full overflow-hidden">
                            <div className="h-full bg-accent rounded-full transition-all duration-700" style={{ width: `${productionPercent}%` }} />
                          </div>
                        </div>
                      </div>
                    )}
                      ...
                      <div className="absolute top-14 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 mt-2 w-32">
                        ...
                        <div className="bg-surface border border-border-custom rounded-2xl p-2 space-y-1.5 w-full shadow-lg">
                          {(['En confección','En empaque'] as OrderStatus[]).map((sub) => {
                            const subCompleted = currentStepIndex > i || (sub === 'En confección' && ['En empaque','En despacho','Entregado'].includes(foundOrder?.status || ''));
                            const subCurrent   = foundOrder?.status === sub;
                            return (
                              <div key={sub} className="flex items-center gap-1.5">
                                <div className={cn("w-4 h-4 rounded-full flex items-center justify-center shrink-0 transition-all", subCompleted || currentStepIndex > i ? "bg-accent text-white" : subCurrent ? "bg-accent/30 border-2 border-accent" : "bg-surface-hover border border-border-custom")}>
                                ...
                                <span className={cn("text-[8px] font-black uppercase tracking-wider", subCompleted || currentStepIndex > i ? "text-accent" : subCurrent ? "text-foreground-main" : "text-foreground-muted/30")}>{sub === 'En confección' ? 'En Confección' : 'En Empaque'}</span>
                              </div>
                            );
                          })}
                        </div>
                    ...
                    <div className="flex flex-col items-center gap-1 mt-0">
                      <span className={cn("text-[9px] font-black uppercase tracking-widest text-center max-w-[90px] leading-tight", isCompleted ? "text-foreground-main" : "text-foreground-muted/50")}>
                        {step}
                      </span>
                    ...
                    <div key={cat} className="space-y-3">
                      ...
                      {ref ? (
                        <div className="aspect-video rounded-3xl overflow-hidden border border-border-custom relative group shadow-lg">
                          <img src={ref.file_path} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" referrerPolicy="no-referrer" />
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4">
                            ...
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                              <a href={ref.file_path} download onClick={(e) => e.stopPropagation()} className="p-4 bg-white/10 backdrop-blur-md rounded-full text-white hover:bg-accent transition-all">
                                <Download size={20} />
                              </a>
                            </div>
                          </div>
              ...
              <div className="bg-foreground-main/[0.02] p-8 rounded-[40px] border border-border-custom shadow-2xl h-fit space-y-8">
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <h4 className="font-black text-foreground-main uppercase tracking-[0.3em] text-[11px] truncate">Propuesta del Diseñador</h4>
                  </div>
                ...
                <div 
                  ...
                  onClick={() => { setSelectedImageUrl(foundOrder.versions![0].file_path); setShowImageModal(true); }}
                >
                  <img src={foundOrder.versions[0].file_path} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" referrerPolicy="no-referrer" />
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <div className="p-4 bg-white/10 backdrop-blur-md rounded-full text-white"><Maximize2 size={24} /></div>
                  </div>
                ...
                {foundOrder.status === 'Versión enviada' && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <button onClick={handleApproveDesign} className="bg-green-600 text-white py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:scale-105 transition-all shadow-xl shadow-green-600/20 flex items-center justify-center gap-3">
                      <CheckCircle size={20} /> Aprobar
                    </button>
                    <button onClick={() => setShowRejectModal(true)} className="bg-accent text-white py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:scale-105 transition-all shadow-xl shadow-accent/20 flex items-center justify-center gap-3">
                ...
                {foundOrder.status === 'Corrección solicitada' && foundOrder.versions[foundOrder.versions.length - 1].client_comments && (
                  <div className="p-6 bg-accent/5 rounded-[24px] border border-accent/10">
                    ...
                    <p className="text-xs text-foreground-main italic leading-relaxed">{foundOrder.versions[foundOrder.versions.length - 1].client_comments}</p>
                    <p className="text-[9px] font-bold text-foreground-muted uppercase tracking-widest mt-4">El diseñador está trabajando en una nueva versión.</p>
                  </div>
                )}
              </div>
            )}
 
            {/* ── Diseño de Uniformes — compacto, ancho completo ── */}
            {foundOrder && (
                  ...
                  <table className="w-full text-left border-collapse min-w-[800px]">
                    <thead>
                      <tr className="border-b border-border-custom">
                        {['Nombre en Camiseta','Número','Talla','Manga','Tipo','Horma','Observaciones'].map(h => (
                          <th key={h} className="pb-6 text-[9px] font-black uppercase tracking-widest text-foreground-muted px-4">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border-custom">
                        ...
                        <tr key={idx} className="group hover:bg-foreground-main/[0.01] transition-colors">
                          <td className="py-6 px-4">
                            {isDesignPhase ? (
                              ...
                              <input type="text" value={item.player_name || ''}
                                onChange={e => { const n=[...editingItems]; n[idx].player_name=e.target.value; setEditingItems(n); }}
                                className={cn("bg-surface border rounded-xl px-4 py-3 text-[11px] w-full focus:border-accent/50 outline-none font-bold uppercase tracking-widest transition-all", (!item.player_name||item.player_name.trim()==='')?"border-red-400/60 bg-red-500/5":"border-border-custom")}
                          ...
                          <td className="py-6 px-4">
                            {isDesignPhase ? (
                              ...
                              <input type="number" value={item.number || ''}
                                onChange={e => { const n=[...editingItems]; n[idx].number=e.target.value; setEditingItems(n); }}
                                className={cn("bg-surface border rounded-xl px-4 py-3 text-[11px] w-full focus:border-accent/50 outline-none font-bold uppercase tracking-widest transition-all", (!item.number||item.number.trim()==='')?"border-red-400/60 bg-red-500/5":"border-border-custom")}
                                placeholder="00" />
                            )}
                          </td>
                          <td className="py-6 px-4">
                            {isDesignPhase ? (
                              ...
                              <select value={item.size || ''} onChange={e => { const n=[...editingItems]; n[idx]={...n[idx],size:e.target.value}; setEditingItems(n); }}
                                className={cn("bg-surface border rounded-xl px-4 py-3 text-[11px] w-full focus:border-accent/50 outline-none font-bold uppercase tracking-widest transition-all", (!item.size||item.size.trim()==='')?"border-red-400/60 bg-red-500/5":"border-border-custom")}>
                                <option value="">Talla</option>
                          ...
                          <td className="py-6 px-4">
                            {isDesignPhase ? (
                              ...
                              <select value={item.sleeve||'MANGA'} onChange={e => { const n=[...editingItems]; n[idx].sleeve=e.target.value; setEditingItems(n); }}
                                className={cn("bg-surface border rounded-xl px-4 py-3 text-[11px] w-full focus:border-accent/50 outline-none font-bold uppercase tracking-widest transition-all", (!item.sleeve||item.sleeve.trim()==='')?"border-red-400/60 bg-red-500/5":"border-border-custom")}>
                                <option value="">MANGA</option>
                                <option value="Corta">Corta</option>
                                <option value="Larga">Larga</option>
                              </select>
                          ...
                          <td className="py-6 px-4">
                            {isDesignPhase ? (
                              ...
                              <select value={item.design_type||'TIPO'} onChange={e => { const n=[...editingItems]; n[idx].design_type=e.target.value; setEditingItems(n); }}
                                className={cn("bg-surface border rounded-xl px-4 py-3 text-[11px] w-full focus:border-accent/50 outline-none font-bold uppercase tracking-widest transition-all", (!item.design_type||item.design_type.trim()==='')?"border-red-400/60 bg-red-500/5":"border-border-custom")}>
                                <option value="">TIPO</option>
                                <option value="Jugador">Jugador</option>
                                <option value="Portero">Portero</option>
                              </select>
                          ...
                          <td className="py-6 px-4">
                            {isDesignPhase ? (
                              ...
                              <select value={item.fit||'HORMA'} onChange={e => { const n=[...editingItems]; n[idx].fit=e.target.value; setEditingItems(n); }}
                                className={cn("bg-surface border rounded-xl px-4 py-3 text-[11px] w-full focus:border-accent/50 outline-none font-bold uppercase tracking-widest transition-all", (!item.fit||item.fit.trim()==='')?"border-red-400/60 bg-red-500/5":"border-border-custom")}>
                                <option value="">HORMA</option>
                                <option value="Hombre">Hombre</option>
                                <option value="Dama">Dama</option>
                              </select>
                          ...
                          <td className="py-6 px-4">
                            {isDesignPhase ? (
                              ...
                              <input type="text" value={item.observations || ''}
                                onChange={e => { const n=[...editingItems]; n[idx].observations=e.target.value; setEditingItems(n); }}
            ...
            {foundOrder.versions && foundOrder.versions.length > 1 && (
              <div className="bg-foreground-main/[0.02] p-8 rounded-[40px] border border-border-custom shadow-2xl col-span-full">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <h4 className="font-black text-foreground-main uppercase tracking-[0.4em] text-[11px] whitespace-nowrap">Historial de Diseños</h4>
                </div>
                  ...
                  {foundOrder.versions.map((v) => (
                    <div key={v.id} className="bg-surface p-6 rounded-[32px] border border-border-custom shadow-xl group">
                      <div className="flex justify-between items-center mb-4">
                        <span className="px-3 py-1 bg-background rounded-full text-[9px] font-black uppercase tracking-widest text-foreground-muted">Versión #{v.version_number}</span>
                        <span className={cn("text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full", v.status === 'Aprobado' ? "bg-green-500/10 text-green-500" : "bg-accent/10 text-accent")}>{v.status}</span>
                      </div>
                      <div className="relative aspect-video rounded-2xl overflow-hidden mb-4 cursor-pointer" onClick={() => { setSelectedImageUrl(v.file_path); setShowImageModal(true); }}>
                        <img src={v.file_path} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" referrerPolicy="no-referrer" />
                      ...
                      {v.client_comments && (
                        <div className="p-3 bg-accent/5 rounded-xl border border-accent/10">
                          <p className="text-[9px] font-black uppercase tracking-widest text-accent mb-1 flex items-center gap-1.5"><MessageSquare size={10} /> Tus Comentarios:</p>
                    ...
                    {(showAllHistory
                      ...
                      return (
                        <div key={i} className="relative pl-8 group w-full">
                          <div className={cn("absolute left-0 top-1.5 w-[15px] h-[15px] rounded-full border-2 transition-colors", isQualityReject ? "bg-amber-500/20 border-amber-500" : "bg-surface border-border-custom group-hover:border-accent")} />
                          <div className="w-full">
                            <p className={cn("text-[11px] font-black tracking-tight leading-tight mb-1", isQualityReject ? "text-amber-500" : "text-foreground-main")}>{h.action}</p>
                            {isQualityReject ? (
                              <div className="w-full bg-amber-500/5 border border-amber-500/20 rounded-xl p-3 mb-2">
                                <p className="text-[9px] font-black uppercase tracking-widest text-amber-500 flex items-center gap-1.5 mb-1"><AlertTriangle size={10} /> Rechazo de calidad</p>
                                <p className="text-[10px] text-foreground-main leading-relaxed break-words">{h.details?.replace('⚠ Rechazo de calidad en impresión — Motivo: ','')}</p>
                              </div>
                            ) : (
                              <p className="w-full text-[10px] text-foreground-muted leading-relaxed mb-2 break-words">{h.details}</p>
                            )}
        ...
        <div className="py-32 text-center bg-surface border border-border-custom shadow-2xl">
          ...
          <h4 className="font-black text-2xl text-foreground-main tracking-tighter mb-4">Busca tu pedido</h4>
          <p className="text-foreground-muted text-[11px] font-black tracking-widest uppercase mb-6">Ingresa tu número de orden para ver el estado en tiempo real</p>
          <div className="bg-accent/5 border border-accent/10 rounded-2xl px-6 py-4 inline-block">
            <p className="text-accent text-[10px] font-black tracking-widest">Ejemplo: ORD-ABC123</p>
          </div>
        </div>
      )}
      ...
      {showThankYouMessage && (
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }}
          ...
          <div className="w-full max-w-md bg-surface rounded-[40px] border border-border-custom p-10 shadow-2xl text-center space-y-6 relative overflow-hidden">
            ...
            <div className="space-y-3">
              <h4 className="text-2xl font-black text-foreground-main tracking-tighter uppercase leading-tight">¡Bachestic está orgulloso de ser parte de tu equipo!</h4>
              <p className="text-foreground-muted text-sm font-bold leading-relaxed">Gracias por preferirnos. Tu información fue guardada exitosamente y ya estamos trabajando para ti.</p>
            </div>
            <button onClick={() => setShowThankYouMessage(false)} className="w-full bg-accent text-white py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:scale-105 transition-all shadow-xl shadow-accent/20">¡Perfecto!</button>
          </div>
        </motion.div>
      )}
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[300] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }}
              ...
              <div className="text-center space-y-6">
                ...
                <div>
                  <h4 className="text-2xl font-black text-foreground-main tracking-tighter uppercase">¿Confirmar Listado?</h4>
                  <p className="text-foreground-muted text-[11px] font-bold uppercase tracking-widest mt-2 leading-relaxed">Una vez confirmado, el listado pasará a producción y <span className="text-accent">ya no podrás modificarlo</span>.</p>
                </div>
                <div className="bg-surface-hover rounded-2xl border border-border-custom p-5 text-left space-y-2">
                  <p className="text-[9px] font-black uppercase tracking-widest text-foreground-muted mb-3">Resumen del listado</p>
                ...
                <div className="flex gap-4 pt-2">
                  <button onClick={() => setShowSaveConfirmModal(false)} className="flex-1 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] border border-border-custom text-foreground-muted hover:bg-surface-hover transition-all">Revisar de nuevo</button>
                  <button onClick={executeSaveItems} disabled={isSavingItems}
                    ...
                    {isSavingItems ? <RefreshCw className="animate-spin" size={16} /> : <CheckCircle2 size={16} />} Sí, confirmar
                  </button>
                </div>
              </div>
            </motion.div>
          ...
          <div className="fixed inset-0 bg-background/80 backdrop-blur-md z-[200] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }}
              ...
              <div>
                ...
                <p className="text-foreground-muted text-[11px] font-black uppercase tracking-widest leading-relaxed">Al confirmar, el pedido pasará al estado de "Abono pendiente" para iniciar el proceso técnico de diseño.</p>
              </div>
              <div className="flex gap-4">
                <button onClick={() => setShowConfirmModal(false)} className="flex-1 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] text-foreground-muted hover:text-foreground-main transition-colors">Cancelar</button>
                <button onClick={handleConfirmOrder} className="flex-1 bg-accent text-white py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-2xl shadow-accent/20 hover:scale-105 transition-all">Confirmar</button>
              </div>
            </motion.div>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-background/80 backdrop-blur-md z-[200] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }}
              ...
              <div className="mb-8">
                ...
                <p className="text-foreground-muted text-[11px] font-black uppercase tracking-widest leading-relaxed">Indica los cambios necesarios para el diseño</p>
              </div>
              <form onSubmit={handleRejectDesign} className="space-y-6">
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-foreground-muted mb-3">Comentarios para el diseñador</label>
                  <textarea value={rejectComment} onChange={(e) => setRejectComment(e.target.value)}
                    ...
                    placeholder="Ej: Cambiar el color del logo..." required />
                </div>
                <div className="flex gap-4 pt-4">
                  <button type="button" onClick={() => setShowRejectModal(false)} className="flex-1 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] text-foreground-muted hover:text-foreground-main transition-colors">Cancelar</button>
                  <button type="submit" disabled={isSubmittingReject || !rejectComment.trim()}
                    ...
                    {isSubmittingReject ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <MessageSquare size={16} />}
                    Enviar Comentarios
                  </button>
                </div>
              </form>
