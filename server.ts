...
async function startServer() {
  ...
  // Migration: Add client_id if not exists
  try {
  ...
  try {
    db.prepare('ALTER TABLE payments ADD COLUMN file_path TEXT').run();
  } catch (e) {}
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS products (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      name TEXT NOT NULL,
      category TEXT NOT NULL, -- camiseta, pantaloneta, medias, chaquetas
      sale_price REAL DEFAULT 0,
    ...
    const transaction = db.transaction(() => {
      ...
      if (items !== undefined) {
        ...
        for (const item of items) {
          insertItem.run(order_id, item.item_name, item.player_name, item.number, item.size, item.sleeve, item.design_type, item.fit, item.garment_type, item.observations, item.sewing_price || 0, item.sale_price || 0, item.design_path || null, item.active !== undefined ? (item.active ? 1 : 0) : 1);
        }
      }
  ...
  app.post('/api/orders/:id/items', (req, res) => {
    ...
    const transaction = db.transaction((items) => {
      const createdItems = [];
      for (const item of items) {
        const info = insert.run(order_id, item.item_name, item.player_name, item.number, item.size, item.sleeve, item.design_type, item.fit, item.garment_type, item.observations, item.sewing_price || 0, item.sale_price || 0, item.design_path || null);
  ...
  app.post('/api/design-versions/:id/status', (req, res) => {
    ...
    const details = status === 'Diseño aprobado' ? 'El cliente aprobó el diseño.' : `El cliente solicitó correcciones: ${client_comments}`;
  ...
  app.post('/api/orders/:id/payments', upload.single('document'), (req: MulterRequest, res) => {
    const { amount, payment_method, reference, notes, user_name } = req.body;
    const file_path = req.file ? `/uploads/${req.file.filename}` : null;
    const order_id = req.params.id;

    const transaction = db.transaction(() => {
      db.prepare(`
        INSERT INTO payments (order_id, amount, payment_method, reference, notes, created_by, file_path)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(order_id, amount, payment_method, reference, notes, user_name || 'Sistema', file_path);

      // Update order status and payment status
      const order = db.prepare('SELECT total_amount, advance_amount, advance_paid FROM orders WHERE id = ?').get(order_id);
      const totalPaid = db.prepare('SELECT SUM(amount) as total FROM payments WHERE order_id = ?').get(order_id).total || 0;

      let paymentStatus = 'Pendiente';
      let advancePaid = order.advance_paid;
      let totalPaidFlag = 0;
      ...
      db.prepare(`
        UPDATE orders 
        SET advance_paid = ?, total_paid = ?, payment_status = ?, advance_amount = ?
        ${status ? ', status = ?' : ''}
        WHERE id = ?
      `).run(advancePaid, totalPaidFlag, paymentStatus, totalPaid, ...(status ? [status, order_id] : [order_id]));

      db.prepare('INSERT INTO order_history (order_id, user_name, action, details) VALUES (?, ?, ?, ?)').run(order_id, user_name || 'Sistema', 'Pago Registrado', `Pago registrado por $${amount}. Total pagado: $${totalPaid}`);
    });
  ...
  app.post('/api/items/:id/design', upload.single('design'), (req: MulterRequest, res) => {
    const file_path = req.file ? `/uploads/${req.file.filename}` : null;
    if (!file_path) return res.status(400).json({ error: 'No file uploaded' });

    db.prepare('UPDATE order_items SET design_path = ? WHERE id = ?').run(file_path, req.params.id);
    res.json({ success: true, path: file_path });
  });
  app.get('/api/stats', (req, res) => {
    ...
    const monthlySales = db.prepare("SELECT SUM(total_amount) as total FROM orders WHERE active = 1 AND created_at >= date('now', 'start of month')").get().total || 0;
    ...
    res.json({ activeOrders, monthlySales, delayedOrders });
  });
